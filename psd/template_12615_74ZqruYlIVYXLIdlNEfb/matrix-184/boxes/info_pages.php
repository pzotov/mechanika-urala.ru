...
$info_categories_query = tep_db_query("select ic.categories_id, icd.categories_name from " . TABLE_PAGES_CATEGORIES . " ic, " . TABLE_PAGES_CATEGORIES_DESCRIPTION . " icd where icd.categories_id = ic.categories_id and icd.language_id = '" . (int)$languages_id . "' and ic.categories_status = '1' order by ic.categories_sort_order, icd.categories_name");

// pages outside categories
$info_pages_query = tep_db_query("select ip.pages_id, ipd.pages_title from " . TABLE_PAGES . " ip, " . TABLE_PAGES_DESCRIPTION . " ipd left join " . TABLE_PAGES_TO_CATEGORIES . " ip2c on ip2c.pages_id = ip.pages_id where ipd.pages_id = ip.pages_id and ip2c.categories_id = '0' and ipd.language_id = '" . (int)$languages_id . "' and ip.pages_status = '1' order by ip.pages_sort_order, ipd.pages_title");
...
// new info2BoxHeading($info_box_contents, false, false,false);

  $info_string = '';
  while ($info_categories = tep_db_fetch_array($info_categories_query)) {
    $id_string = 'cID=' . $info_categories['categories_id'];
    $info_string .= '<a href="' . tep_href_link(FILENAME_PAGES, $id_string) . '">' . $info_categories['categories_name'] . '</a><br>';
  }

  while ($info_pages = tep_db_fetch_array($info_pages_query)) {
    $id_string = 'pID=' . $info_pages['pages_id'];
    $info_string .= '<a href="' . tep_href_link(FILENAME_PAGES, $id_string) . '">' . $info_pages['pages_title'] . '</a><br>';
  }
...
$info_box_contents = array();
  ...
  new info3box($info_box_contents, true, true);
