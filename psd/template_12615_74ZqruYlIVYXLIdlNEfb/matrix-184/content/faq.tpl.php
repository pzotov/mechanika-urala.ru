...
table_image_border_top(false, false, $faq_category['categories_name']);
}
?>
...
<?php
  ...
  if ((tep_db_num_rows($faq_categories_query) > 0) || (tep_db_num_rows($faq_query) > 0)) {
    while ($faq_categories = tep_db_fetch_array($faq_categories_query)) {
      echo '<span class="faqTitle"><b>' . '<a href="' . tep_href_link(FILENAME_FAQ, 'cID=' . (int)$faq_categories['categories_id']) . '">' . $faq_categories['categories_name'] . '</a>' . '</b></span>' . "\n";
      echo '<p class="faqBlurb">' . $faq_categories['categories_description'] . '</p>' . "\n\n";
    }

    while ($faq = tep_db_fetch_array($faq_query)) {
      echo '<span class="faqTitle"><b>' . '<a href="' . tep_href_link(FILENAME_FAQ, 'pID=' . (int)$faq['faq_id']) . '">' . $faq['question'] . '</a>' . '</b></span>' . "\n";
